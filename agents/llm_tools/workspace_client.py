...
LOGGER = logging.getLogger("argus.agents.llm_tools.workspace")


def _response(action: str, payload: str, context: dict[str, Any]) -> dict[str, Any]:
    LOGGER.info("Workspace client action=%s payload=%s context=%s", action, payload, context)
    return {"status": "ok", "tool": "workspace", "action": action, "output": f"Workspace stub response for: {payload}"}
